...
// pattern may be:
// a function that takes two: (element, index), one: (element) or any arguments (similar to map
...
// Note: Sequences are great to work with functional methods like map, reduce, forEach, every or
// any. For example:

// // sum of numbers 1-10
// let sum = sequence(10, (x, idx) => idx+1).reduce((sum, num) => sum + num);
// Be careful with long sequances. They are just arrays, every element is created when function is
// called.

const sequence = (n, pattern) => (typeof pattern === 'function'
  ? Array(n).fill().map(pattern)
